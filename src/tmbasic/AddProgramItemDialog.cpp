...
static const std::vector<std::string> kAddProgramItemStrings{
    "F1 Subroutine", "F2 Function", "F3 Shared variable", "F4 Constant",
    "F5 Type",       "F6 Form",     "F7 Custom control",  "F8 Picture",
};

static const std::unordered_map<uint16_t, uint16_t> kAddProgramItemCommands{
...
static const std::vector<uint16_t> kAddProgramItemKeys{
...
class AddProgramItemListBox : public ListViewer {
   public:
    AddProgramItemListBox(const TRect& bounds, uint16_t numCols) : ListViewer(bounds, numCols, nullptr, nullptr) {
        setRange(static_cast<int16_t>(kAddProgramItemStrings.size()));
    }
    ...
    void selectItem(int16_t item) override {
        message(owner, static_cast<uint16_t>(evCommand), cmOK, nullptr);
