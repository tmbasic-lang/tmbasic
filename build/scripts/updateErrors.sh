#!/bin/bash
# updates BuiltInConstantList.cpp and builtin_constants.txt based on the errors in src/shared/Error.h
...
grep ERR_ src/shared/Error.h | awk '{ print "    addError(\"" $5 "\", shared::ErrorCode::" $1 ");" }' >> /tmp/list2.cpp
tail -n +$(($LINE_NUMBER+1)) /tmp/list.cpp >> /tmp/list2.cpp
mv -f /tmp/list2.cpp src/compiler/BuiltInConstantList.cpp
rm -f /tmp/list.cpp

# doc/help/topics/builtin_constants.txt
