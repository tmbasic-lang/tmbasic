...
echo >> $OUTFILE
echo '#include "shared/SystemCalls.h"' >> $OUTFILE
...
echo '        return;' >> $OUTFILE
echo '    }' >> $OUTFILE
echo $SYSTEM_CALLS | awk 'BEGIN { RS = " " } { print "    initSystemCall(shared::SystemCall::k" $1 ", systemCall" $1 ");" }' >> $OUTFILE
echo '    _systemCallsInitialized = true;' >> $OUTFILE
echo '}' >> $OUTFILE
echo >> $OUTFILE
echo '}  // namespace vm' >> $OUTFILE

OUTFILE=src/shared/SystemCalls.h
