...
pushd $PUBLISHDIR/tmbasic/bin && tar zcf $PUBLISHDIR/tmbasic-linux-arm64.tar.gz tmbasic LICENSE.txt LICENSE.tmbasic.txt && popd

./linux-arm32.sh -ic "$CLEAN_CMD"
pushd $PUBLISHDIR/tmbasic/obj && tar zxf $PUBLISHDIR/runners.tar.gz && popd
./linux-arm32.sh -ic "$BUILD_CMD"
pushd $PUBLISHDIR/tmbasic/bin && tar zcf $PUBLISHDIR/tmbasic-linux-arm32.tar.gz tmbasic LICENSE.txt LICENSE.tmbasic.txt && popd
